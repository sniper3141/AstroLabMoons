...
hdul = fits.open('/Users/jameshartman/Desktop/AstroLabMoons/Stacked_Images/mosaic.fits')
data = hdul[0].data
header = hdul[0].header
hdul.close()

Z = data[372:396, 534:558].astype(float)     #372:396, 534:558
print(Z)
# ny, nx = 120, 150
ny, nx = Z.shape
...
chi2_xz, dof_xz, chi2r_xz, p_xz, N_xz = chi2_stats(
    obs_xz, model_xz, sigma_xz, k_params_used=K_PARAMS_FOR_SLICE
)
chi2_yz, dof_yz, chi2r_yz, p_yz, N_yz = chi2_stats(
    obs_yz, model_yz, sigma_yz, k_params_used=K_PARAMS_FOR_SLICE
)
...
for i in range(len(location_of_smallest_diff)):
    model.append(z_x_section[location_of_smallest_diff[i]])
model.append(26)
# print(model)
# print(z_x_data)
...
res_x = (z_x_data - model)/sigma_x            #we need to compare the residual to the model at that specific pixal
...
model = []
for i in range(len(location_of_smallest_diff)):
    model.append(z_y_section[location_of_smallest_diff[i]])
model.append(26)
# print(model)
# print(z_x_data)
...
res_y = (abs(z_y_data) - model)/sigma_y
